...
def test_get_images_term(image_folder):
    ...
    cmd = ['python', script_dir , '--dir', str(image_folder), '--output', 'all_images.npy']
    ...
    assert process_jpg.stdout == b'Images saved in array of array of size (1, 3, 200, 200, 3)\n'
    ...
    assert process_resized.stdout == b'Images saved in array of array of size (1, 3, 32, 32, 3)\n'

    # check that the images are converted to grayscale (that means the last dimension is dropped, i.e. 4 -> 3)
    cmd.extend(['--gray_scale', 'True'])
    process_gray = subprocess.run(cmd, capture_output=True)
    assert process_gray.returncode == 0
    assert process_gray.stdout == b'Images saved in array of array of size (1, 3, 32, 32)\n'
