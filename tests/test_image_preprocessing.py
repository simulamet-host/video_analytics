...
def test_get_images_call(image_folder):
    ...
    # First dim is the number of images, second and third are the image dimensions,
    # last is the number of channels.
    opt_.gray_scale = True
    images_gray, _ = e2e_img_pre.get_images(opt_)
    # check that the imgages are converted to grayscale (that means the last dimension
    # is dropped, i.e. 5 -> 4)
...
def test_get_images_term(image_folder):
    ...
    assert (
        process_jpg.stdout
        == b"Images saved in array of array of size (1, 3, 200, 200, 3)\n"
    )
    ...
    assert (
        process_resized.stdout
        == b"Images saved in array of array of size (1, 3, 32, 32, 3)\n"
    )

    # check that the images are converted to grayscale (that means the last dimension is
    # dropped, i.e. 4 -> 3)
    ...
    assert (
        process_gray.stdout
        == b"Images saved in array of array of size (1, 3, 32, 32)\n"
    )
