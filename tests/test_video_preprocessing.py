...
class TestVideoPreprocessor(unittest.TestCase):
    ...
    def test_process_video(self, mock_get_video_files, mock_cv2_capture):
        ...
        mock_cv2_capture_instance.get.side_effect = itertools.chain(
            [24.0, 10.0, 1.0, False], itertools.repeat(False)
        )
        ...
        config = VideoConfig(
            ...
            output_folder="test_videos/frames/",
            save_frames=True,
        )
        processor = VideoPreprocessor(config)
        ...
        calls = [
            call(f"test_videos/frames/test_video/frame{index}.jpg", "test_frame")
