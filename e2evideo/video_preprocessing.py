...
class VideoPreprocessor:
    ...
    def get_video_files(self):
        ...
        assert os.path.isdir(self.config.videos_folder), 'The given videos_folder does not exist'
        self.create_folder(self.config.output_folder)
        search_path = os.path.join(self.config.videos_folder, '**', '*' + self.config.video_format)
    ...
    def calculate_frame_indices(self, frame_rate, total_num_frames, video_file):
        ...
        if self.config.num_frames > total_num_frames:
            ...
            frame_indices = [round(frame_rate * i) for i in range(self.config.num_frames)]
    ...
    def interpolate_missing_frames(self, frame_data: FrameData):
        ...
        while len(frame_data.frames) < self.config.num_frames:
            ...
            interpolated_frame = cv2.addWeighted(frame_data.frame, 1 - alpha, frame_data.frames[-1],
                                                 alpha, 0)
    ...
    def process_video(self):
        ...
        for video_file in video_files:
            ...
            frame_data.frames_folder = os.path.join(self.config.output_folder, images_sub_folder)
            ...
            frame_data.frame_indices = self.calculate_frame_indices(frame_data.frame_rate,
                                                total_num_frames, video_file)
            frame_data.count = 0
            frame_data.frames = []
            ...
            while cap.isOpened():
                ...
                if self.config.back_sub is not None:
                    masked_frame_name = f"fgMask_frame{frame_data.count}."+self.config.image_format
                ...
                if self.config.sampling_mode == 'every_frame':
                    ...
                    should_save_frame = frame_id % math.floor(frame_data.frame_rate) == 0
                elif cond1:
                    should_save_frame = frame_id in frame_data.frame_indices
                if should_save_frame:
                    cv2.imwrite(frame_data.frames_folder + '/' + file_name, frame_data.frame) # pylint: disable=E1101
                    if masked_frame_name is not None:
                        # Apply the mask to the original frame
                        # pylint: disable=E1101
                        fg_frame = cv2.bitwise_and(frame_data.frame, frame_data.frame, mask=fg_mask)
                        cv2.imwrite(frame_data.frames_folder + '/' + masked_frame_name, fg_frame)
                    frame_data.count += 1
                ...
                if cond1 and cond2 and cond3:
                    print((f"Warning: the number of frames extracted from {video_file} is"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sampling_mode', default='every_frame',
