        ...
        for video_file in video_files:
            ...
            while cap.isOpened():
                ...
                if self.config.sampling_mode == 'every_frame':
                    ...
                    if frame_id % math.floor(frame_data.frame_rate) == 0:
                        cv2.imwrite(frame_data.frames_folder + '/'  + file_name, frame_data.frame) # pylint: disable=E1101
                        frame_data.count += 1
                elif cond1:
                    if frame_id in frame_data.frame_indices:
                        # pylint: disable=E1101
                        cv2.imwrite(frame_data.frames_folder + '/' + file_name, frame_data.frame)
                        frame_data.count += 1
...
if __name__ == '__main__':
    ...
    videos_config = VideoConfig(opts.videos_folder, opts.video_format, opts.image_format,
                            opts.sampling_mode, opts.num_frames, opts.output_folder)
