...
class VideoPreprocessing:
    ...
    def get_frames(self):
        ...
        """
        self.paths['selected_folder'] = self.folder_selector_ui(self.paths['starting_path'])
        self.cmd_list = ['python', '../video_preprocessing.py', '--videos_folder',
        ...
        self.cmd_list.extend(['--video_format', self.options['video_format'], '--image_format',
                              self.options['image_format'], '--sampling_mode',
                              self.options['sampling_mode']])

        if self.options['sampling_mode'] == 'fixed_frames':
            self.options['num_frames'] = st.number_input('Number of frames', min_value=1,
        ...
        if st.session_state.extract_frames_clicked:
            if st.button('Show Example Frames', key='show_example_button'):
                ...
                for subdir in random_frames_subdir:
                    frames = glob.glob(os.path.join(self.paths['output_folder'],
                                                    subdir, "*." + self.options['image_format']))
                    frames.sort()
