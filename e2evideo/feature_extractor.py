...
def main(args_):
    ...
    if args_.dataset_name == "CIFAR10":
        ...
        visual_data, test_data = train_test_split(
            test_data, test_size=0.98, random_state=42
        )
    elif args_.dataset_name == "action_recognition":
        x_train, x_test, _, _, _ = load_ucf101.load_ucf101(
            args_.data_folder, args_.images_array, args_.no_classes
        )
        # pylint: disable=E1101
        x_train, x_test = torch.tensor(x_train).to(device), torch.tensor(x_test).to(
            device
        )
        ...
        train_dataset, test_dataset = load_ucf11.get_data(
            args_.labels_file, args_.images_array
        )
    if args_.mode == "train":
        #  training model
        model = conv_autoencoder.ConvolutionalAutoencoder(
