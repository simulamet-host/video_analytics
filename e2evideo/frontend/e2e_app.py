...
colored_text(
    "Please place your data files in a folder inside the 'data' folder,\
...
def folder_selector_ui(input_path):
    ...
    selected_dir = st.selectbox(
        "Select video folder", st.session_state.subdirs, key="video_folder_selectbox"
    )
    column = st.columns(3)
...
with st.expander("Video Preprocessing"):
    ...
    image_format = st.selectbox(
        "Select image format",
        ("jpg", "png", "bmp", "tiff", "gif", "webp", "ico", "raw", "eps", "psd", "svg"),
    ...
    if st.session_state.extract_frames_clicked:
        if st.button("Show Example Frames", key="show_example_button"):
            ...
            for subdir in random_frames_subdir:
                frames = glob.glob(
                    os.path.join(output_folder, subdir, "*." + image_format)
                )
                frames.sort()
